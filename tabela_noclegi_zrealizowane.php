...
if(isset($_SESSION['user_id']) || isset($_COOKIE['ciastko_user_id'])){
	...
	if (isset($_POST['filtry_szukaj'])){
		...
		$query = "SELECT * FROM panel_tabela_noclegi WHERE zaznaczony_wiersz = 0 AND ( okres_wynajmu_do != '' AND okres_wynajmu_do != ' ' AND '".date('Y-m-d')."' > SUBSTRING(okres_wynajmu_do,1,10));";
	}
