...
if((isset($_SESSION['user_id']) && $_SESSION['user_type'] == 'a') || (isset($_COOKIE['ciastko_zalogowany']) && $_COOKIE['ciastko_zalogowany'] == 'a')){
	...
	if(isset($_POST['powrot'])){
		...
		header('Location: tabela_noclegi_skopiowano.php?id='.mysql_insert_id().$filters_path);
		
	}else{
		echo '<form action="tabela_noclegi_kopiuj.php?id='.$id.$filters_path.'" method="post">	
		<table width="100%">';
		if(isset($_SESSION['user_id'])){
		...
		echo '<tr '.kolor_wiersza_noclegi($row['okres_wynajmu_do'], $row['zaznaczony_wiersz']).'>
				...
				<td id="komorka_edytowanie"><textarea name="firma" cols="10" rows="2">'.$row['firma'].'</textarea></td>
				...
				<td id="komorka_edytowanie"><textarea name="placone" cols="5" rows="2">'.$row['placone'].'</textarea></td>
