...
if(isset($_SESSION['user_id']) || isset($_COOKIE['ciastko_user_id'])){
	...
	if (isset($_POST['zwyzki_zmien_limit'])) {
		...
		$queryUpdateLimit = "UPDATE panel_users SET zwyzki_limit = ".$_POST['limit']." WHERE id = ".$userId.";";
		mysql_query($queryUpdateLimit);
	}
	...
	if (isset($_POST['filtry_szukaj'])){
		...
		$query = "SELECT * FROM panel_tabela_zwyzki ".$limit." ;";
	}
	...
	echo '</div><br /><br /><br /><br /><br /><br /><br /><br /><br /><br /><br /><br /><br /><br /><br /><br /><br /><br /><br /><br /><br /><br />';
	echo '<div>'; 
	
	// Formularz stworzony na potrzeby exportu danych do excel.
	echo '<form action="tabela_zwyzki_excel.php" method="post">	';
	
	if($row_header['header'] == 'n'){
		echo '<input type="submit" value="Excel" name="zwyzki_excel" /><br /><br />';
	} else {
		echo '<input type="submit" value="Excel" name="zwyzki_excel" /><br /><br />';
	}
	...
	while($row = mysql_fetch_array($result,MYSQL_ASSOC)){
		...
		echo '			
				...
				if($_SESSION['user_type'] == 'a' || $_COOKIE['ciastko_zalogowany'] == 'a'){
					if($row_header['id'] != 16){ //Nie widzi user PDG
					echo '<td id="komorka_edycja"><a href="tabela_zwyzki_edytuj.php?id='.$row['id'].$filters_path.'"><button type="button">E</button></a></td>
						<td id="komorka_kopiuj"><a href="tabela_zwyzki_kopiuj.php?id='.$row['id'].$filters_path.'"><button type="button">K</button></a></td>
						<td id="komorka_usun"><a href="tabela_zwyzki_usun.php?id='.$row['id'].$filters_path.'"><button type="button">X</button></a></td>';
					}	
				}
			echo '</tr>
		';
