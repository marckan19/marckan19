...
if(isset($_SESSION['user_id']) || isset($_COOKIE['ciastko_user_id'])){
	...
	<table>
		<tr>';
			if($row_header['header'] == 'n'){
				echo '<td><h2>Baustellen - Mitarbeiter</h2></td><td width="20px" />';			
			}else{
				echo '<td><h2>Wyjazdy - PRACOWNICY</h2></td><td width="20px" />';			
			}
			//require_once('legenda_wyjazdy.php');
		echo '</tr>
	</table>';
	
	require 'tabela_wyjazdy_pracownicy_filtry.php';
	if (isset($_POST['filtry_szukaj'])){
		...
		$query = "SELECT u.login AS login, ptn.* FROM panel_tabela_wyjazdy_pracownicy ptn
					JOIN panel_users u ON u.id = ptn.creator_id
			WHERE 
			ptn.nazwisko like '%".$_POST['filtry_nazwisko']."%' &&	
			ptn.imie like '%".$_POST['filtry_imie']."%' &&						
			ptn.stanowisko like '%".$_POST['filtry_stanowisko']."%' &&			
			ptn.kontakt like '%".$_POST['filtry_kontakt']."%' &&			
			ptn.creation_date like '%".$_POST['filtry_data_wpisu']."%' &&
			u.login like '%".$_POST['filtry_login']."%' &&
			ptn.is_deleted = 0
			ORDER BY ptn.nazwisko
		;";
		
		$filters_path = '&nazwisko='.$_POST['filtry_nazwisko'].'				
						&imie='.$_POST['filtry_imie'].'								
						&stanowisko='.$_POST['filtry_stanowisko'].'																			
		...
		$query = "SELECT u.login AS login, ptz.* FROM panel_tabela_wyjazdy_pracownicy ptz
					JOIN panel_users u ON u.id = ptz.creator_id 
			WHERE 
			ptz.nazwisko like '%".$_GET['nazwisko']."%' &&			
			ptz.imie like '%".$_GET['imie']."%' &&			
			ptz.stanowisko like '%".$_GET['stanowisko']."%' &&			
			ptz.kontakt like '%".$_GET['kontakt']."%' &&			
			ptz.creation_date like '%".$_GET['data_wpisu']."%' &&
			u.login like '%".$_GET['login']."%'
			&& ptz.is_deleted = 0
			ORDER BY ptz.nazwisko
		;";
		
		$filters_path = '&nazwisko='.$_GET['nazwisko'].'	
						&imie='.$_GET['imie'].'											
		...
		$query = "SELECT * FROM panel_tabela_wyjazdy_pracownicy 
		WHERE is_deleted = 0
		ORDER BY nazwisko;";
	}
	...
	if($_SESSION['user_type'] == 'a' || $_COOKIE['ciastko_zalogowany'] == 'a'){
		...
		if($row_header['header'] == 'n'){
			echo '<a href="tabela_wyjazdy_pracownicy_dodaj.php'.$filters_path2.'"><button>neuer Mitarbeiter</button></a><br /><br />';
		}else{
			echo '<a href="tabela_wyjazdy_pracownicy_dodaj.php'.$filters_path2.'"><button>Dodaj pracownika</button></a><br /><br />';
		}
	}
	...
	while($row = mysql_fetch_array($result,MYSQL_ASSOC)){
		...
		echo '			
				...
				if($_SESSION['user_type'] == 'a'){
					echo '<td id="komorka_edycja"><a href="tabela_wyjazdy_pracownicy_edytuj.php?id='.$row['id'].$filters_path.'"><button>E</button></a></td>
						 <td id="komorka_usun"><a href="tabela_wyjazdy_pracownicy_ukryj.php?id='.$row['id'].$filters_path.'"><button>X</button></a></td>';
				}
			echo '</tr>
		';
		$lp++;
	...
	if($_SESSION['user_type'] == 'a'){
		...
		if($row_header['header'] == 'n'){
			echo '<br /><a href="tabela_wyjazdy_pracownicy_dodaj.php'.$filters_path2.'"><button>neuer Mitarbeiter</button></a>';
		}else{
			echo '<br /><a href="tabela_wyjazdy_pracownicy_dodaj.php'.$filters_path2.'"><button>Dodaj pracownika</button></a>';
		}
	}
}else{
