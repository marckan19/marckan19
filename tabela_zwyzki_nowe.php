...
if(isset($_SESSION['user_id']) || isset($_COOKIE['ciastko_user_id'])){
	...
	if (isset($_POST['filtry_szukaj'])){
		...
		$query = "SELECT * FROM panel_tabela_zwyzki WHERE (data_zdania = '' OR data_zdania = ' ') AND zaznaczony_wiersz = 0 AND oferta = 0 AND (zawieszone_do = '' OR zawieszone_do = ' ' OR '".date('Y-m-d')."' >= substr(zawieszone_do, 1, 10));";
	}
	...
	if($row_header['header'] == 'n'){
		...
		echo '
			...
			<td id="naglowek" rowspan="2" width="85px">ZAWIE. DO</td>
