...
if(isset($_SESSION['user_id']) || isset($_COOKIE['ciastko_user_id'])){
	...
	<table>
		<tr>';
			...
			//<td><h2>Noclegi</h2></td><td width="20px" />';			
	...
	if (isset($_POST['filtry_szukaj'])){
		...
		$query = "SELECT * FROM panel_tabela_noclegi WHERE zaznaczony_wiersz = 0 AND (okres_wynajmu_do = NULL or okres_wynajmu_do = '' or okres_wynajmu_do = ' ' or '".date('Y-m-d')."' <= SUBSTRING(okres_wynajmu_do,1,10));";
	}
