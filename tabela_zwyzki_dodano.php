...
if ((isset($_SESSION['user_id']) && $_SESSION['user_type'] == 'a') || (isset($_COOKIE['ciastko_zalogowany']) && $_COOKIE['ciastko_zalogowany'] == 'a')) {
    ...
    // Używamy przygotowanego zapytania do pobrania danych z tabeli
    $stmt = $mysqli->prepare("SELECT * FROM panel_tabela_zwyzki WHERE id = ?");
    ...
    // WYCIAGANIE ID USERA
    $stmt_login = $mysqli->prepare("SELECT * FROM panel_users WHERE id = ?");
    ...
    if (isset($_SESSION['user_id'])) {
        $stmt_header = $mysqli->prepare("SELECT * FROM panel_users WHERE id = ?");
