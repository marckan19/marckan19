...
if((isset($_SESSION['user_id']) && $_SESSION['user_type'] == 'a') || (isset($_COOKIE['ciastko_zalogowany']) && $_COOKIE['ciastko_zalogowany'] == 'a')){
	...
	if($row_header['header'] == 'n'){
		echo '<h2>Baustellen - Mitarbeiter löschen</h2>';
	}else{
		echo '<h2>Wyjazdy - usuwanie pracownika</h2>';
	}
	echo '</div><br /><br /><br /><br /><br /><br /><br /><br /><br />';
		
		if($row_header['header'] == 'n'){
			echo '<div id="success">Mitarbeiter wurde gelöscht</div><br />';
		}else{
			echo '<div id="success">Pracownik został usunięty.</div><br />';
		}
