...
if((isset($_SESSION['user_id']) && $_SESSION['user_type'] == 'a') || (isset($_COOKIE['ciastko_zalogowany']) && $_COOKIE['ciastko_zalogowany'] == 'a')){
	...
	<table>
		<tr>';
			if($row_header['header'] == 'n'){
				echo '<td><h2>Bauvorhaben - den Eintrag bearbeiten</h2></td><td width="20px" />';			
			}else{
	...
	if(isset($_POST['powrot'])){
		...
		$query_update = "UPDATE panel_tabela_budowy
						SET
		...
		header('Location: tabela_budowy_edytowano.php?id='.$id.$filters_path);
		
	}else{
		echo '<form action="tabela_budowy_edytuj.php?id='.$id.$filters_path.'" method="post">	
		<table width="100%">
			<tr>
				...
				<td id="naglowek" rowspan="2">LAND/<br />KRAJ</td>
			</tr>
			<tr>
		...
		echo '<tr '.kolor_wiersza_budowy($row['realizacja_od'], $row['realizacja_do'], $row['zaznaczony_wiersz']).'>
				...
				<td id="komorka_edytowanie"><textarea name="kierownik" cols="9" rows="2">'.$row['kierownik'].'</textarea></td>
				<td id="komorka_edytowanie"><textarea name="dokumenty" cols="13" rows="3">'.$row['dokumenty'].'</textarea></td>
				<td id="komorka_edytowanie"><textarea name="tydzien" cols="13" rows="2">'.$row['tydzien'].'</textarea></td>
				...
				<td id="komorka_dodawanie">
					<select name="kraj">
						<option value=""></option>
						<option value="h"'; if ($row['kraj'] == 'h') { echo 'selected'; } echo '>Holandia</option>
						<option value="n"'; if ($row['kraj'] == 'n') { echo 'selected'; } echo '>Niemcy</option>
					</select>
				</td>		
			</tr>
		';
