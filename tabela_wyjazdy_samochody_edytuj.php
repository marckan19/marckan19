...
if((isset($_SESSION['user_id']) && $_SESSION['user_type'] == 'a') || (isset($_COOKIE['ciastko_zalogowany']) && $_COOKIE['ciastko_zalogowany'] == 'a')){
	...
	<table>
		<tr>';		
			if($row_header['header'] == 'n'){
				echo '<td><h2>Baustellen - Dienstwagen bearbeiten</h2></td><td width="20px" />';			
			}else{
				echo '<td><h2>Wyjazdy - edytowanie samochodu</h2></td><td width="20px" />';			
			}
	...
	if(isset($_POST['powrot'])){
		...
		$query_update = "UPDATE panel_tabela_wyjazdy_samochody
						SET
							samochod = '".$samochod."', 
		...
		header('Location: tabela_wyjazdy_samochody_edytowano.php?id='.$id.$filters_path);
		
	}else{
		echo '<form action="tabela_wyjazdy_samochody_edytuj.php?id='.$id.$filters_path.'" method="post">	
		<table>';
		if($row_header['header'] == 'n'){
			echo '<tr>
		...
		echo '<tr '.kolor_wiersza_wyjazdy_samochody().'>
				<td id="komorka_edytowanie"><input name="samochod" value="'.$row['samochod'].'" /></td>
				<td id="komorka_edytowanie"><input name="rejestracja" value="'.$row['rejestracja'].'" /></td>
