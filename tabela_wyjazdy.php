...
if(isset($_SESSION['user_id']) || isset($_COOKIE['ciastko_user_id'])){
	...
	<table>
		<tr>';
			if($row_header['header'] == 'n'){
				echo '<td><h2>Baustellen</h2></td><td width="20px" />';			
			}else{
				echo '<td><h2>Wyjazdy</h2></td><td width="20px" />';			
			}
	...
	if (isset($_POST['filtry_szukaj'])){
		...
		$query = "
			...
			LEFT JOIN panel_tabela_wyjazdy_pracownicy_powiazania ptwpp ON ptwpp.wpis_id = ptww.id
			LEFT JOIN panel_tabela_wyjazdy_pracownicy ptwp ON ptwp.id = ptwpp.pracownik_id
			LEFT JOIN panel_tabela_wyjazdy_budowy_powiazania ptwbp ON ptwbp.wpis_id = ptww.id
			LEFT JOIN panel_tabela_wyjazdy_budowy ptwb ON ptwb.id = ptwbp.budowa_id
			LEFT JOIN panel_tabela_wyjazdy_samochody_powiazania ptwsp ON ptwsp.wpis_id = ptww.id
			LEFT JOIN panel_tabela_wyjazdy_samochody ptws ON ptws.id = ptwsp.samochod_id
			JOIN panel_users u ON u.id = ptww.creator_id
			WHERE 
			(ptwp.imie like '%".$_POST['filtry_pracownik']."%' ||			
			ptwp.nazwisko like '%".$_POST['filtry_pracownik']."%') &&
			ptwb.budowa like '%".$_POST['filtry_budowa']."%' &&		
			/*ptws.rejestracja like '%".$_POST['filtry_samochod']."%' &&	*/
			(ptwbp.zawiesic_1 like '%".$_POST['filtry_zawiesic']."%' ||			
			ptwbp.zawiesic_2 like '%".$_POST['filtry_zawiesic']."%' ||			
			ptwbp.zawiesic_3 like '%".$_POST['filtry_zawiesic']."%' ||		
			ptwbp.zawiesic_4 like '%".$_POST['filtry_zawiesic']."%' ||			
			ptwbp.zawiesic_5 like '%".$_POST['filtry_zawiesic']."%' ||		
			ptwbp.zawiesic_6 like '%".$_POST['filtry_zawiesic']."%' ||			
			ptwbp.zawiesic_7 like '%".$_POST['filtry_zawiesic']."%') &&			
			(ptwbp.zdac_1 like '%".$_POST['filtry_zdac']."%' ||			
			ptwbp.zdac_2 like '%".$_POST['filtry_zdac']."%' ||			
			ptwbp.zdac_3 like '%".$_POST['filtry_zdac']."%' ||			
			ptwbp.zdac_4 like '%".$_POST['filtry_zdac']."%' ||			
			ptwbp.zdac_5 like '%".$_POST['filtry_zdac']."%' ||			
			ptwbp.zdac_6 like '%".$_POST['filtry_zdac']."%' ||			
			ptwbp.zdac_7 like '%".$_POST['filtry_zdac']."%')&&			
			(ptwbp.zamowic_1 like '%".$_POST['filtry_zamowic']."%' ||		
			ptwbp.zamowic_2 like '%".$_POST['filtry_zamowic']."%' ||		
			ptwbp.zamowic_3 like '%".$_POST['filtry_zamowic']."%' ||		
			ptwbp.zamowic_4 like '%".$_POST['filtry_zamowic']."%' ||		
			ptwbp.zamowic_5 like '%".$_POST['filtry_zamowic']."%' ||		
			ptwbp.zamowic_6 like '%".$_POST['filtry_zamowic']."%' ||		
			ptwbp.zamowic_7 like '%".$_POST['filtry_zamowic']."%') &&	
			ptwbp.budowa_od like '%".$_POST['filtry_budowa_od']."%' &&
			ptwbp.budowa_do like '%".$_POST['filtry_budowa_do']."%' &&	
			ptwbp.uwagi like '%".$_POST['filtry_uwagi']."%' &&			
			ptww.creation_date like '%".$_POST['filtry_data_wpisu']."%' &&			
			u.login like '%".$_POST['filtry_login']."%'	&& 
			ptww.archiwizacja = 0			
			GROUP BY ptww.id
		";		
			
		$filters_path = '&pracownik='.$_POST['filtry_pracownik'].'
						&budowa='.$_POST['filtry_budowa'].'						
						&samochod='.$_POST['filtry_samochod'].'						
						&budowa_od='.$_POST['filtry_budowa_od'].'						
						&budowa_do='.$_POST['filtry_budowa_do'].'												
						&zawiesic='.$_POST['filtry_zawiesic'].'												
						&zdac='.$_POST['filtry_zdac'].'												
						&zamowic='.$_POST['filtry_zamowic'].'												
	...
	if($_SESSION['user_type'] == 'a' || $_COOKIE['ciastko_zalogowany'] == 'a'){
		...
		if($row_header['header'] == 'n'){
			echo '<a href="tabela_wyjazdy_dodaj.php'.$filters_path2.'"><button>neuer Eintrag</button></a><br /><br />';
		}else{
			echo '<a href="tabela_wyjazdy_dodaj.php'.$filters_path2.'"><button>Dodaj wpis</button></a><br /><br />';
		}
		}
	}
	...
	<table width="100%">
		...
		if ($row_zalogowany['header'] == 'n'){
			...
			if($row_header['id'] != 16){ //Nie widzi user PDG
				echo'					
					<td id="naglowek" rowspan="1">E</td>
					<td id="naglowek" rowspan="1">U</td>';
			} 
			} 
		echo '</tr>';
	$lp = 1;
	...
	while($row = mysql_fetch_array($result,MYSQL_ASSOC)){
		...
		echo '			
				...
				if($_SESSION['user_type'] == 'a' || $_COOKIE['ciastko_zalogowany'] == 'a'){
				if($row_header['id'] != 16){ //Nie widzi user PDG
					//echo '<td id="komorka_edycja"><a href="tabela_wyjazdy_edytuj.php?id='.$row['id'].$filters_path.'"><button>E</button></a></td>';
					echo '<td id="komorka_edycja"><a href="tabela_wyjazdy_dodatkowa_budowa_dodaj.php?id='.$row['id'].$filters_path.'"><button>+</button></a></td>';
					echo '<td id="komorka_usun"><a href="tabela_wyjazdy_usun.php?id='.$row['id'].$filters_path.'"><button>X</button></a></td>';
				}
				}
			echo '</tr>
		';
		...
		foreach($wyniki as $wiersz){
			$przekaz = $przekaz . $lp_tablicy . '=' . $wiersz . '&';
			$lp_tablicy++;
		}
	...
	if($_SESSION['user_type'] == 'a' || $_COOKIE['ciastko_zalogowany'] == 'a'){
		...
		if($row_header['header'] == 'n'){
			...
			echo '<br /><a href="tabela_wyjazdy_dodaj.php'.$filters_path2.'"><button>neuer Eintrag</button></a> ';
			echo '<a href="tabela_wyjazdy_archiwizuj.php'.$filters_path2.'"><button>Archiwizuj</button></a><br /><br />';
		}else{
			echo '<br /><a href="tabela_wyjazdy_dodaj.php'.$filters_path2.'"><button>Dodaj wpis</button></a> ';
			echo '<a href="tabela_wyjazdy_archiwizuj.php'.$przekaz.'"><button>Archiwizuj</button></a><br /><br />';
		}
		}
	}
}else{
