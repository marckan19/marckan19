...
if(isset($_SESSION['user_id']) || isset($_COOKIE['ciastko_user_id'])){
	...
	if (isset($_POST['filtry_szukaj'])){
		...
		$query = "SELECT * FROM panel_tabela_budowy WHERE zaznaczony_wiersz = 0 AND '".date('Y-m-d')."' >= SUBSTRING(realizacja_od,1,10) AND '".date('Y-m-d')."' <= SUBSTRING(realizacja_do,1,10) ORDER BY realizacja_od;";
	}
