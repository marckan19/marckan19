...
if(isset($_SESSION['user_id']) || isset($_COOKIE['ciastko_user_id'])){
	...
	<table>
		<tr>';
			if($row_header['header'] == 'n'){
				echo '<td><h2>Baustellen - Archiwum</h2></td><td width="20px" />';			
			}else{
				echo '<td><h2>Wyjazdy - Archiwum</h2></td><td width="20px" />';			
			}
			require_once('legenda_wyjazdy.php');
		echo '</tr>
	</table>';
	
	require 'tabela_wyjazdy_filtry_archiwum.php';
	...
	if (isset($_POST['filtry_szukaj'])){
		$query = "
			...
			WHERE 			
			ptwb.id = '".$_POST['filtry_budowa_id']."' &&	
			ptww.archiwizacja = 1			
			GROUP BY ptww.id
		";		
