...
if(isset($_SESSION['user_id']) || isset($_COOKIE['ciastko_user_id'])){
	...
	require 'tabela_wyjazdy_filtry_historia.php';
	if (isset($_POST['filtry_szukaj'])){
		...
		$query = "
			SELECT ptww.* 
			FROM panel_tabela_wyjazdy_wpisy ptww ORDER BY ptww.creation_date			
		";			
	}
	...
	while($row = mysql_fetch_array($result,MYSQL_ASSOC)){
		...
		if($stara_budowa == $row['budowa_id']){
			$lp_uzytkownika++;			
		}elseif($row['budowa_id'] == 0 || $row['budowa_id'] == ''){
			$lp_uzytkownika = 1;			
		}else{
			$lp_uzytkownika = 1;
			$stara_budowa = $row['budowa_id'];
		}
