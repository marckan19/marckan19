...
if((isset($_SESSION['user_id']) && $_SESSION['user_type'] == 'a') || (isset($_COOKIE['ciastko_zalogowany']) && $_COOKIE['ciastko_zalogowany'] == 'a')){
	...
	if(isset($_POST['powrot'])){
		...
		$query_insert = "INSERT INTO panel_tabela_wyjazdy_pracownicy (imie, nazwisko, stanowisko, kontakt, creator_id, creation_date, modificator_id, modification_date)
						VALUES (
							'".$imie."', 
							'".$nazwisko."', 						
							'".$stanowisko."', 							
		...
		header('Location: tabela_wyjazdy_pracownicy_dodano.php?id='.mysql_insert_id().$filters_path);
		
	}else{
		echo '<form action="tabela_wyjazdy_pracownicy_dodaj.php'.$filters_path.'" method="post">	
		<table>';
		if($row_header['header'] == 'n'){
			echo '<tr>
		...
		echo '		
				<td id="komorka_dodawanie"><input name="imie" /></td>
				<td id="komorka_dodawanie"><input name="nazwisko" /></td>
				<td id="komorka_dodawanie"><input name="stanowisko" /></td>
				<td id="komorka_dodawanie"><input name="kontakt" /></td>
			';
		echo '</tr>';
