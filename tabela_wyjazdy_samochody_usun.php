...
if((isset($_SESSION['user_id']) && $_SESSION['user_type'] == 'a') || (isset($_COOKIE['ciastko_zalogowany']) && $_COOKIE['ciastko_zalogowany'] == 'a')){
	...
	<table>
		<tr>';		
			if($row_header['header'] == 'n'){
				echo '<td><h2>Baustellen - Dienstwagen löschen</h2></td><td width="20px" />';			
			}else{
				echo '<td><h2>Wyjazdy - usuwanie samochodu</h2></td><td width="20px" />';			
			}
	...
	if(isset($_POST['usun'])){
		...
		$query_delete = "DELETE FROM panel_tabela_wyjazdy_samochody WHERE id = ".$id.";";
		...
		header('Location: tabela_wyjazdy_samochody_usunieto.php'.$filters_path);
		...
		echo '<form action="tabela_wyjazdy_samochody_usun.php?id='.$id.$filters_path.'" method="post">	
		<table>';
		if($row_header['header'] == 'n'){
			echo '<tr>
		...
		echo '
				<tr '.kolor_wiersza_wyjazdy_samochody().'>
		...
		if($row_header['header'] == 'n'){
			echo '</table><br />Möchten Sie Dienstwagen löschen?<br /><br /><input type="submit"  value="Ja" name="usun" /> <input type="submit"  value="Nein" name="nie_usuwaj" /></form>';	
		}else{
			echo '</table><br />Czy na pewno chcesz usunąć ten samochód?<br /><br /><input type="submit"  value="Tak" name="usun" /> <input type="submit"  value="Nie" name="nie_usuwaj" /></form>';	
		}
	}
}else{
