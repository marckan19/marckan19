...
if(isset($_SESSION['user_id']) || isset($_COOKIE['ciastko_user_id'])){
	...
	if (isset($_POST['filtry_szukaj'])){
		...
		$query = "SELECT u.login AS login, ptn.* FROM panel_tabela_budowy ptn
			...
			ptn.usterka_zgloszenie like '%".$_POST['filtry_usterka_zgloszenie']."%' &&
			ptn.usterka_usuniecie like '%".$_POST['filtry_usterka_usuniecie']."%' &&
			ptn.usterka_osoba like '%".$_POST['filtry_usterka_osoba']."%' &&
		...
		$filters_path = '&zleceniodawca='.$_POST['filtry_zleceniodawca'].'
						...
						&usterka_zgloszenie='.$_POST['filtry_usterka_zgloszenie'].'
						&usterka_usuniecie='.$_POST['filtry_usterka_usuniecie'].'
						&usterka_osoba='.$_POST['filtry_usterka_osoba'].'						
		...
		$query = "SELECT u.login AS login, ptz.* FROM panel_tabela_budowy ptz
			...
			ptz.usterka_zgloszenie like '%".$_GET['usterka_zgloszenie']."%' &&
			ptz.usterka_usuniecie like '%".$_GET['usterka_usuniecie']."%' &&
			ptz.usterka_osoba like '%".$_GET['usterka_osoba']."%' &&
		...
		$filters_path = '&zleceniodawca='.$_GET['zleceniodawca'].'
						...
						&usterka_zgloszenie='.$_GET['usterka_zgloszenie'].'
						&usterka_usuniecie='.$_GET['usterka_usuniecie'].'
						&usterka_osoba='.$_GET['usterka_osoba'].'
		...
		$tydzien_przed = date('Y-m-d',strtotime(date('Y-m-d').' +7 day'));
		$query = "SELECT * FROM panel_tabela_budowy WHERE zaznaczony_wiersz = 0 AND '".$tydzien_przed."' >= SUBSTRING(realizacja_od,1,10) AND '".date('Y-m-d')."' < SUBSTRING(realizacja_od,1,10) ORDER BY realizacja_od;";
	}
