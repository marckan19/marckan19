...
if((isset($_SESSION['user_id']) && $_SESSION['user_type'] == 'a') || (isset($_COOKIE['ciastko_zalogowany']) && $_COOKIE['ciastko_zalogowany'] == 'a')){
	...
	if(isset($_POST['powrot'])){
		...
		header('Location: tabela_usterki_skopiowano.php?id='.mysql_insert_id().$filters_path);
		
	}else{
		echo '<form action="tabela_usterki_kopiuj.php?id='.$id.$filters_path.'" method="post">	
		<table>';
		...
		if($row_header['header'] == 'n'){
			...
			<tr>
				...
				<td id="naglowek" width="155px">gemeldet am <input type="checkbox" name="czy_data_zgloszenia" value="1" /></td>
				<td id="naglowek" width="155px">Beseitigungsfrist<input type="checkbox" name="czy_termin_usuniecia" value="1" /></td>
				<td id="naglowek" width="155px">beseitigt am <input type="checkbox" name="czy_data_usuniecia" value="1" /></td>																		
